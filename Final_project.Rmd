---
title: "Data Visualization of Breast Cancer"
author: "Francis Osei"
date: "17/12/2021"
output:
...
melt_cor %>% 
  ...
  geom_text(aes(Var2, Var1, label = value), color = "black", size = 2,alpha=0.7 )
```
...
We plotted a quasirandom beeswarm to distinguished between Begign and Malignant. For each plot we have 10 features. The are some features that are difficult to differentiate between Begign and Malignant. This techniques provides a visible and easy to interpret since each data point do not overlap.
...
cancer %>% 
  ggplot(aes(x=compactness_worst,y = radius_worst,colour = diagnosis)) +
  geom_point(size = 3) +
  stat_ellipse(aes(fill = diagnosis,color = diagnosis), geom="polygon",level=0.95,alpha=0,type = "norm", linetype = 2,beta=0.5)+
  scale_color_manual(values = c("darkorange","cyan4"))
